...
Select Count(BOOK_TITLE) as NO_OF_BOOKS from LMS_BOOK_DETAILS  where BOOK_TITLE like '%java%';
...
--Problem 8
--Write a query to display the book code, book title of the books which are issued on the date "1
--st April 2012".
...
SELECT CONCAT(BOOK_TITLE,'_is_written_by_',AUTHOR) AS NAME from LMS_BOOK_DETAILS;
...
--account.
Select SUPPLIER_ID,SUPPLIER_NAME,EMAIL FROM LMS_SUPPLIERS_DETAILS WHERE LEN(EMAIL) IN 
(SELECT MIN(LEN(EMAIL)) FROM LMS_SUPPLIERS_DETAILS);
...
--Select SUPPLIER_ID,SUPPLIER_NAME ,CONTACT from LMS_SUPPLIERS_DETAILS;
