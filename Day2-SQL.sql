...
ALTER Table Users
ADD CONSTRAINT Category Default 'ADMIN' for Category;
...
--Create a Check constraint for the Transaction Type as it should accept the only the IN/OUT values

Alter table [Transaction]
add constraint chk_TransactionType check (TransactionType in('IN','OUT'));
...
-- = is called as single row comparsion operator

Select * from ProductMaster where AvailableQty=500 or AvailableQty=1000 or AvailableQty=1020;

Select * from ProductMaster where AvailableQty!=500;

--IN operator is Multiple row comparsion operator
Select * from ProductMaster where AvailableQty in(500,1000,1020);

Select * from ProductMaster where AvailableQty not in(500,1000,2000);

Delete from Users where USERID > 3;


--User Name start with M
Select * from Users where UserName like 'M%';

--User Name having second letter as 'i'

Select * from Users where UserName like '_i%';

Select * from Users where UserName like 'a%';

Select * from Users where UserEmail like '%gmail.com';

Select * from Users where UserEmail like '%gmail%';

--Aggregrate Operators
...
--1.Write a query to display the id, entire street address and the first word of the street address of all entries in the location table. 
--Give an alias to the first word as 'first_word'.
--Display the records sorted in ascending order based on first_word.(Q10)

--Select id , street_address 
select id , street_address, SUBSTRING(street_address,1,CHARINDEX(' ',street_address)) as first_word from location order by first_word ASC;
...
select id,street_address,Reverse(SUBSTRING(Reverse(street_address),1,CHARINDEX(' ',street_address))) as 'last_word'
...
--Display the records sorted in ascending order based on first name.(Q11).
   ...
   Select first_name,left(replicate('$',5)+convert(char,salary),10) as Salary
...
select first_name from employee where id in( select employee_id from manager where department_id in (select id from department where name='HR'));


--Display the emloyee details whose age is > 25.

select first_name, DATEDIFF(year,date_of_birth,GETDATE()) as 'age' from employee;